# Appendix A: Exit Destinations

X = exclude client from measure entirely. Checkmark = include as positive/acceptable destination. Blank = include in universe but not as positive destination. Undefined project types should use "HP & PH (all)" column.

## Homeless Situations (100-199)

| Code | Destination | SO | ES-EE | ES-NbN | TH | HP & PH (all) | SH | SSO |
|------|------------|:--:|:-----:|:------:|:--:|:-------------:|:--:|:---:|
| 101 | Emergency shelter (incl. hotel/motel paid w/ ES voucher, Host Home shelter) | + | | | | | | |
| 116 | Place not meant for habitation | | | | | | | |
| 118 | Safe Haven | + | | | | | | |

## Institutional Situations (200-299)

| Code | Destination | SO | ES-EE | ES-NbN | TH | HP & PH (all) | SH | SSO |
|------|------------|:--:|:-----:|:------:|:--:|:-------------:|:--:|:---:|
| 206 | Hospital or other residential non-psychiatric medical facility | X | X | X | X | X | X | X |
| 215 | Foster care home or group home | + | X | X | X | X | X | X |
| 207 | Jail, prison, or juvenile detention facility | | | | | | | |
| 204 | Psychiatric hospital or other psychiatric facility | + | | | | | | |
| 205 | Substance abuse treatment facility or detox center | + | | | | | | |
| 225 | Long-term care facility or nursing home | + | X | X | X | X | X | X |

## Temporary Housing Situations (300-399)

| Code | Destination | SO | ES-EE | ES-NbN | TH | HP & PH (all) | SH | SSO |
|------|------------|:--:|:-----:|:------:|:--:|:-------------:|:--:|:---:|
| 329 | Residential project or halfway house (no homeless criteria) | X | | | | | | |
| 314 | Hotel/motel paid w/o ES voucher | + | | | | | | |
| 312 | Staying/living with family, temporary | + | | | | | | |
| 313 | Staying/living with friends, temporary | + | | | | | | |
| 302 | Transitional housing | + | + | | | | | |
| 327 | HOPWA funded project to HOPWA TH | + | | | | | | |
| 332 | Host Home (non-crisis) | + | + | + | + | | | |

## Permanent Housing Situations (400-499)

| Code | Destination | SO | ES-EE | ES-NbN | TH | HP & PH (all) | SH | SSO |
|------|------------|:--:|:-----:|:------:|:--:|:-------------:|:--:|:---:|
| 426 | HOPWA funded to HOPWA PH | + | + | + | + | + | + | + |
| 411 | Owned by client, no subsidy | + | + | + | + | + | + | + |
| 421 | Owned by client, with subsidy | + | + | + | + | + | + | + |
| 410 | Rental by client, no subsidy | + | + | + | + | + | + | + |
| 435 | Rental by client, with housing subsidy | + | + | + | + | + | + | + |
| 422 | Staying/living with family, permanent | + | + | + | + | + | + | + |
| 423 | Staying/living with friends, permanent | + | + | + | + | + | + | + |

## Other (1-99)

| Code | Destination | SO | ES-EE | ES-NbN | TH | HP & PH (all) | SH | SSO |
|------|------------|:--:|:-----:|:------:|:--:|:-------------:|:--:|:---:|
| 24 | Deceased | X | X | X | X | X | X | X |
| 8 | Client doesn't know | | | | | | | |
| 9 | Client prefers not to answer | | | | | | | |
| 99 | Data not collected | | | | | | | |
| 30 | No exit interview completed | | | | | | | |
| 17 | Other | | | | | | | |
